...
buttonQuestion.forEach((item) => {
    ...
    document.addEventListener("click", function () {
        ...
        console.log(list_myPopover.length);
        if (
            list_myPopover.length > 1 && event.target !== item //điểm targer không phải là button
        ) {
            list_myPopover.forEach((item2) => {
                item2.remove();
            });
            popover = null;
        } else if ((list_myPopover.length = 1 && event.target !== item)) {
            list_myPopover.forEach((item2) => {
                item2.remove();
            });
            popover = null;
