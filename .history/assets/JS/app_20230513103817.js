...
let dataLanguage = [
    {ID:"VN", language: "Vietnames", flagUrl: "./assets/img/vietnam-flag-png-large.png"},
    {ID:"EN", Language: "English", flagUrl: "./assets/img/united-kingdom-flag-png-large.png"}
];
    ...
    if(elm.matches('li.countryItem') ){
        ...
        if(elm.matches('li.VN')) {
            dataLanguage.forEach(item => {
                console.log(dataLanguage[item].ID);
                if(dataLanguage[item].ID === "VN") {
                    console.log(dataLanguage[item]);
                }
            });
        }
    }else {
