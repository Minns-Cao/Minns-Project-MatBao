...
buttonQuestion.forEach((item) => {
    ...
    document.addEventListener("click", function () {
        ...
        console.log(event.target.classList.contains("buttonQuestion"));
        if (
            list_myPopover.length > 1 &&
            !event.target.classList.contains("buttonQuestion") //điểm targer không phải là button
        ) {
            ...
            list_myPopover.length > 1 &&
            event.target.classList.contains("buttonQuestion")
        ) {
            console.log("do dai : " + list_myPopover.length);
            for (let i = 0; i < list_myPopover.length - 1; i++) {
                list_myPopover[i] = list_myPopover[i+1]
            }
            popover = null;
        }
    });
