...
buttonQuestion.forEach((item) => {
    ...
    item.addEventListener("click", () => {
        ...
        let button = item;
        //nếu như đã có popover thì xoá popover cũ đi ( chỉ duy nhất 1 xuất hiện)
        let checkMyPopover = document.querySelector(".myPopover");
        console.log(checkMyPopover);
        console.log(event.currentTarget);
        console.log(button);
        console.log(checkMyPopover && event.currentTarget !== button);
        console.log(event.currentTarget !== button);
        if (checkMyPopover && event.currentTarget !== button) {
            checkMyPopover.remove();
        }
    ...
    document.addEventListener("click", function () {
        ...
        if (
            list_myPopover.length > 0 &&
            event.target !== item //điểm targer không phải là button
        ) {
            list_myPopover.forEach((i))
            popover = null;
        }
    });
});
