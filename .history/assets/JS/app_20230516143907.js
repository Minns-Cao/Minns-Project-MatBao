...
buttonQuestion.forEach((item) => {
    ...
    item.addEventListener("click", () => {
        ...
        if (popoverPosition === "top") {
            ...
            //popover hướng lên trên, nên phải set bottom = -5 cho hình tam giác
            let bottom = -5 + "px";
            document.body.style.setProperty('--valueTopPopover', bottom);
        } else if (popoverPosition === "bottom") {
            popover.style.top = popoverTop + "px";
            popover.style.left = popoverLeft + "px";
            //popover hướng xuống dưới, nên phải set bottom = -5 cho hình tam giác
            let top = -5 + "px";
            document.body.style.setProperty('--valueBottomPopover', top);
        }
